...
void insertKVPairInJson::insertKVPairInJsonEveryDepth(QJsonObject &obj)
{
    ...
    if (!childObjectsKeys.isEmpty())
    {
        ...
        this->insertKVPairInJsonEveryDepth(childObj);
...
void insertKVPairInJson::randomInsertKVPairInJsonRandomDepth(QJsonObject &obj, double stopProbability)
{
    ...
    else
    {
        ...
        if (!childObjectsKeys.isEmpty())
        {
            QString selectedKey = childObjectsKeys.at(this->randGenerator->bounded(childObjectsKeys.size()));
            QJsonObject childObj = obj[selectedKey].toObject();
            this->randomInsertKVPairInJsonRandomDepth(childObj, stopProbability);
            obj.insert(selectedKey, childObj);
        }
        else;
        QString jsonKey = this->getRandomStr(5);
